using Badget.Algebra.Matrix;
using System;
using System.Collections.Generic;
using System.Linq.Expressions;
using System.Text;

namespace Badget.Win.Temp
{
    internal static class ObjectiveFunction
    {
        public static string CreateFunctionText(int zoneCount, double[,] costs, double[] tripRowSums, double[] tripColumnSums)
        {
            ...
            for (int constraintIndex = 0; constraintIndex < constraintCount; constraintIndex++)
            {
                ...
                {
                    int ai = FindArrayAIndex(constraintIndex, zoneCount, constraintCount);
                    stringBuilder.Append($"x[{ai}]");

                    for (int coj = 0; coj < zoneCount; coj++)
                    {
                        int coi = constraintIndex % zoneCount;
                        int bi = FindArrayBIndex(constraintIndex, zoneCount, constraintCount, coj);
                ...
                if (constraintIndex + 1 != constraintCount)
                {
                    stringBuilder.Append(" + ");
                }
